...
docker volume rm $(docker volume ls -q)rm -rf ./master/data/*
rm -rf ./slave/data/*
...
until docker-compose exec $slave_container_name sh -c 'export MYSQL_PWD=111; mysql -u root -e ";"'
do
    echo "Waiting for "$slave_container_name" database connection..."
    sleep 4
done
...
start_slave_cmd+='"'
docker exec "$slave_container_name" sh -c "$start_slave_cmd"

docker exec "$slave_container_name" sh -c "export MYSQL_PWD=111; mysql -u root -e 'SHOW SLAVE STATUS \G'"

slave_container_name="mysql_slave_2"
until docker-compose exec $slave_container_name sh -c 'export MYSQL_PWD=111; mysql -u root -e ";"'
do
    echo "Waiting for "$slave_container_name" database connection..."
    sleep 4
